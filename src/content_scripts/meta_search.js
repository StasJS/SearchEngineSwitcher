...
function googleMetaSearch(metaSearchAnchors) {
	const searchForm = Array.from(document.forms).find((f) =>
		f.action.endsWith("/search")
	);
	const searchInput = Array.from(searchForm.getElementsByTagName("input")).find(
		(i) => i.name === "q"
	);
