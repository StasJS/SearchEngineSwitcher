...
function useMediaPreference(): MediaPreference | undefined {
  ...
  useEffect(() => {
    const darkPreferenceQuery = window.matchMedia(
      '(prefers-color-scheme: dark)'
    );
    const lightPreferenceQuery = window.matchMedia(
    ...
    const handlerFactory =
      (mp: MediaPreference) => (event: MediaQueryListEvent) => {
        if (event.matches) {
    ...
    return () => {
      ...
      lightPreferenceQuery.removeEventListener(
        'change',
        lightPreferenceHandler
      );
    };
  }, []);
